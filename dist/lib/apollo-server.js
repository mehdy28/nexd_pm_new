...
export default startServerAndCreateNextHandler(server, {
    context: async (req) => {
        ...
        const plainHeaders = {}; // For logging purposes if desired
        if (authHeader)
            plainHeaders["authorization"] = authHeader; // Only put authHeader if it exists for logging
        ...
        console.log("Auth Header Received (from .authorization):", authHeader ? "[PRESENT]" : "[ABSENT]", `Value type: ${typeof authHeader}`);
        ...
        if (token) {
            try {
                ...
                if (prismaUser) {
                    ...
                    const name = prismaUser.firstName || prismaUser.lastName
