...
export const accountResolver = {
    Query: {
        getMyNotificationSettings: async (_, __, context) => {
            if (!context.user) {
        ...
        getWorkspaceAuditLogs: async (_, { workspaceId, skip, take }, context) => {
            if (!context.user) {
                throw new GraphQLError("User is not authenticated", {
                    extensions: { code: "UNAUTHENTICATED" },
                });
    ...
    Mutation: {
        updateMyProfile: async (_, { firstName, lastName, avatar, avatarColor }, context) => {
            if (!context.user) {
                throw new GraphQLError("User is not authenticated", {
                    extensions: { code: "UNAUTHENTICATED" },
                });
            ...
            if (avatarColor !== undefined)
                dataToUpdate.avatarColor = avatarColor;
        ...
        updateMyNotificationSettings: async (_, { input }, context) => {
            if (!context.user) {
                throw new GraphQLError("User is not authenticated", {
                    extensions: { code: "UNAUTHENTICATED" },
                });
