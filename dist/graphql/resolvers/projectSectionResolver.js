...
export const projectSectionResolver = {
    Mutation: {
        createProjectSection: async (_parent, args, context) => {
            if (!context.user?.id) {
        ...
        updateProjectSection: async (_parent, args, context) => {
            if (!context.user?.id) {
                throw new Error("Authentication required: No user ID found in context.");
            }
            const userId = context.user.id;
        ...
        deleteProjectSection: async (_parent, args, context) => {
            if (!context.user?.id) {
                throw new Error("Authentication required: No user ID found in context.");
            }
            const userId = context.user.id;
            ...
            try {
                ...
                let returnedSection = section; // The section to return (either deleted one or new default)
        ...
        reorderProjectSections: async (_, { projectId, sections }, context) => {
            const userId = context.user.id;
            ...
            try {
                ...
                await prisma.$transaction(sections.map(section => prisma.section.updateMany({
                    where: {
                        id: section.id,
