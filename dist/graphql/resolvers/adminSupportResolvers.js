...
export const adminSupportResolvers = {
    // --- QUERIES ---
    Query: {
        adminGetSupportTickets: async (_, __, context) => {
        ...
        adminGetTicketDetails: async (_, { id, cursor, limit = MESSAGES_PAGE_SIZE }, context) => {
    ...
    // --- MUTATIONS ---
    Mutation: {
        adminSendTicketMessage: async (_, { ticketId, content }, context) => {
        ...
        adminUpdateTicketStatus: async (_, { ticketId, status }, context) => {
        ...
        adminUpdateTicketPriority: async (_, { ticketId, priority }, context) => {
        ...
        adminMarkTicketAsRead: async (_, { ticketId }, context) => {
    ...
    Subscription: {
        adminTicketAdded: {
            subscribe: withFilter(() => pubsub.asyncIterableIterator([Topics.ADMIN_TICKET_ADDED]), (payload, variables, context) => {
                return true;
            }),
        },
        adminTicketUpdated: {
            subscribe: withFilter(() => pubsub.asyncIterableIterator([Topics.ADMIN_TICKET_UPDATED]), (payload, variables, context) => {
                return true;
            }),
        },
    }
