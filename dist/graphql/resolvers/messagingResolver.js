...
const getAdminTicketListItemPayload = async (ticketId) => {
...
const createCommunicationListItemPayload = async (item, type, userId) => {
    if (type === 'conversation') {
        // Filter out users who have left for the participant info string
        const activeParticipants = item.participants.filter((p) => !p.hasLeft);
        const participantInfo = item.type === 'DIRECT'
            ? `${item.participants.find((p) => p.userId !== userId)?.user.firstName} ${item.participants.find((p) => p.userId !== userId)?.user.lastName}`.trim()
...
export const messagingResolvers = {
    // --- QUERIES ---
    Query: {
        getWorkspaceMembers: async (_, { workspaceId }, context) => {
        ...
        getCommunicationList: async (_, { workspaceId }, context) => {
            ...
            try {
                ...
                if (!member)
                    throw new GraphQLError('Access denied to workspace resources');
                ...
                const conversationItems = await Promise.all(conversations.map(async (c) => {
                ...
                const ticketItems = await Promise.all(tickets.map(async (t) => {
        ...
        getConversation: async (_, { id, cursor, limit = MESSAGES_PAGE_SIZE }, context) => {
        ...
        getTicket: async (_, { id, cursor, limit = MESSAGES_PAGE_SIZE }, context) => {
            ...
            try {
                ...
                const isSaasAdmin = context.user?.role === 'ADMIN';
                if (ticket.creatorId !== userId && !isSaasAdmin)
                    throw new GraphQLError('Access denied');
                // Fetch paginated messages
    ...
    // --- MUTATIONS ---
    Mutation: {
        userIsTyping: async (_, { conversationId }, context) => {
        ...
        createDirectConversation: async (_, { input }, context) => {
            ...
            try {
                ...
                if (userId === participantId)
                    throw new GraphQLError('Cannot create a direct conversation with yourself.');
        ...
        createGroupConversation: async (_, { input }, context) => {
            ...
            try {
                ...
                if (allParticipantIds.length < 2)
                    throw new GraphQLError('Group conversations require at least two participants.');
                const conversation = await prisma.conversation.create({
                    data: {
                        workspaceId,
                        name,
        ...
        createTicket: async (_, { input }, context) => {
        ...
        sendTicketMessage: async (_, { input }, context) => {
        ...
        markConversationAsRead: async (_, { conversationId }, context) => {
        ...
        markTicketAsRead: async (_, { ticketId }, context) => {
        ...
        addParticipants: async (_, { conversationId, participantIds }, context) => {
            ...
            try {
                ...
                await prisma.$transaction(participantIds.map(id => prisma.conversationParticipant.upsert({
        ...
        leaveConversation: async (_, { conversationId }, context) => {
        ...
        removeParticipant: async (_, { conversationId, userId }, context) => {
            ...
            try {
                ...
                if (conversation.creatorId !== currentUserId)
                    throw new GraphQLError('Only the group creator can remove participants.');
                await prisma.conversationParticipant.update({
                    where: { conversationId_userId: { conversationId, userId } },
                    data: { hasLeft: true, leftAt: new Date() }
                });
    ...
    Subscription: {
        participantRemoved: {
            subscribe: withFilter(() => pubsub.asyncIterableIterator(['PARTICIPANT_REMOVED']), (_payload, _, context) => {
        ...
        communicationItemAdded: {
            subscribe: withFilter(() => pubsub.asyncIterableIterator([Topics.COMMUNICATION_ITEM_ADDED]), async (payload, variables, context) => {
                const userId = context?.user?.id;
                if (!userId)
                    return false;
        ...
        typingUser: {
            subscribe: withFilter(() => pubsub.asyncIterableIterator([Topics.USER_IS_TYPING]), (payload, variables, context) => {
                const loggedInUserId = context?.user?.id;
                if (!loggedInUserId)
                    return false;
                if (loggedInUserId === payload.typingUser.id)
                    return false;
                if (payload.typingUser.conversationId !== variables.conversationId)
                    return false;
                return true;
            }),
        },
        ticketMessageAdded: {
            ...
            subscribe: withFilter(() => pubsub.asyncIterableIterator([Topics.TICKET_MESSAGE_ADDED]), (payload, variables, context = { prisma }) => {
                const isAdmin = context.user?.role === 'ADMIN';
                if (isAdmin) {
                    return true;
                }
        ...
        messageAdded: {
            ...
            subscribe: withFilter(() => pubsub.asyncIterableIterator([Topics.MESSAGE_ADDED]), async (payload, variables, context) => {
                const userId = context?.user?.id;
                if (!userId)
                    return false;
                const participant = await prisma.conversationParticipant.findFirst({
