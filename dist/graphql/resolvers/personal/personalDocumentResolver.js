...
const personalDocumentResolvers = {
    Query: {
        getMyDocuments: async (_parent, { search, skip = 0, take = 12 }, context) => {
            const { user } = context;
            if (!user?.id) {
    ...
    Mutation: {
        createPersonalDocument: async (_parent, { input }, context) => {
            const { user } = context;
        ...
        createDocumentComment: async (_parent, { documentId, content }, context) => {
            const { user } = context;
            if (!user?.id) {
                throw new GraphQLError("Authentication required", { extensions: { code: "UNAUTHENTICATED" } });
            }
        ...
        deleteDocumentComment: async (_parent, { id }, context) => {
            const { user } = context;
            if (!user?.id) {
                throw new GraphQLError("Authentication required", { extensions: { code: "UNAUTHENTICATED" } });
            }
