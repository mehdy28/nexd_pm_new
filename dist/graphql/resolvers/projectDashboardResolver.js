...
export const projectDashboardResolvers = {
    Query: {
        getProjectDashboardData: async (_parent, { projectId, sprintId }, { user }) => {
            if (!user) {
            ...
            if (!startDate || !endDate) {
                if (tasks.length > 0) {
                    startDate = tasks.reduce((earliest, task) => (task.createdAt < earliest ? task.createdAt : earliest), tasks[0].createdAt);
