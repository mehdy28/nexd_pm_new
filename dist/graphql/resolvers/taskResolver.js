...
export const taskResolver = {
    ...
    Mutation: {
        createProjectTask: async (_parent, { input }, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
        ...
        updateProjectTask: async (_parent, args, context) => {
        ...
        deleteProjectTask: async (_parent, args, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
            const userId = context.user.id;
        ...
        deleteManyProjectTasks: async (_parent, { ids }, context) => {
            if (!context.user?.id) {
        ...
        createTaskComment: async (_parent, args, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
        ...
        deleteTaskComment: async (_parent, args, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
            const userId = context.user.id;
        ...
        getAttachmentUploadSignature: async (_parent, args, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
        ...
        confirmAttachmentUpload: async (_parent, { input }, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
        ...
        deleteAttachment: async (_parent, args, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
            const userId = context.user.id;
        ...
        createGanttTask: async (_parent, { input }, context) => {
        ...
        updateGanttTask: async (_parent, { input }, context) => {
            if (!context.user?.id)
                throw new Error("Authentication required.");
    ...
    Task: {
        ...
        attachmentCount: (parent) => {
            return prisma.attachment.count({ where: { taskId: parent.id } });
        },
    },
    TaskListView: {
        completed: (parent) => parent.status === "DONE",
        dueDate: (parent) => {
            if (!parent.dueDate)
                return null;
