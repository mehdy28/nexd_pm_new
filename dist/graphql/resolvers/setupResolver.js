...
export const setupResolver = {
    Mutation: {
        setupWorkspace: async (_parent, args, context) => {
            const { userId, workspaceName, workspaceDescription, projectName, projectDescription, industry, teamSize, workFields, } = args;
            try {
                ...
                const workspace = await prisma.$transaction(async (tx) => {
                    ...
                    const slug = workspaceName.toLowerCase().replace(/\s+/g, "-").slice(0, 50) +
                        "-" +
