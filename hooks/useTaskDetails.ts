  ...
  const addComment = useCallback(async (content: string) => {
    ...
    return createCommentMutation({
      ...
      update: (cache, { data: { createTaskComment } }) => {
        const existingData = cache.readQuery<{ task: TaskDetailsUI }>({
          query: GET_TASK_DETAILS_QUERY,
          variables: { id: taskId },
        });

        if (existingData?.task) {
          cache.writeQuery({
            query: GET_TASK_DETAILS_QUERY,
            variables: { id: taskId },
            data: {
              task: {
                ...existingData.task,
                comments: [...existingData.task.comments, createTaskComment],
              },
            },
          });
        }
          ...
          if (existingData?.task) {
            cache.writeQuery({
              ...
              data: {
                task: {
                  ...existingData.task,
                  attachments: [...existingData.task.attachments, confirmAttachmentUpload],
                },
              },
            });
          }
