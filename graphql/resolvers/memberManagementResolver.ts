...
const checkWorkspaceAdmin = async (workspaceId: string, userId: string, prisma: typeof prisma) => {
...
const checkProjectAdmin = async (projectId: string, userId: string, prisma: typeof prisma) => {
...
export const memberManagementResolvers = {
  Query: {
    getWorkspaceInvitations: async (_: any, { workspaceId }: { workspaceId: string }, context: GraphQLContext) => {
  ...
  Mutation: {
    inviteWorkspaceMembers: async (_: any, { workspaceId, invitations }: { workspaceId: string, invitations: {email: string, role: WorkspaceRole}[] }, context: GraphQLContext) => {
      const inviterId = context.user?.id;
      if (!inviterId) throw new ForbiddenError("Authentication required.");
    ...
    acceptWorkspaceInvitation: async (_: any, { token }: { token: string }, context: GraphQLContext) => {
      const userId = context.user?.id;
      const userEmail = context.user?.email;
      if (!userId || !userEmail) throw new ForbiddenError("Authentication required. Please log in or register.");
    ...
    revokeWorkspaceInvitation: async (_: any, { invitationId }: { invitationId: string }, context: GraphQLContext) => {
        const userId = context.user?.id;
        if (!userId) throw new ForbiddenError("Authentication required.");
        
        const invitation = await prisma.workspaceInvitation.findUnique({ where: { id: invitationId } });
        if (!invitation) throw new UserInputError("Invitation not found.");
    ...
    updateWorkspaceMemberRole: async (_: any, { memberId, role }: { memberId: string, role: WorkspaceRole }, context: GraphQLContext) => {
        const userId = context.user?.id;
        if (!userId) throw new ForbiddenError("Authentication required.");

        const memberToUpdate = await prisma.workspaceMember.findUnique({ where: { id: memberId } });
        if (!memberToUpdate) throw new UserInputError("Member not found.");

        await checkWorkspaceAdmin(memberToUpdate.workspaceId, userId, context.prisma);

        if (memberToUpdate.role === 'OWNER') throw new ForbiddenError("Cannot change the role of the workspace owner.");
        if (role === 'OWNER') throw new ForbiddenError("Cannot assign OWNER role. Transfer ownership instead.");
    ...
    removeWorkspaceMembers: async (_: any, { memberIds }: { memberIds: [string] }, context: GraphQLContext) => {
        const userId = context.user?.id;
        if (!userId) throw new ForbiddenError("Authentication required.");
    ...
    addProjectMembers: async (_: any, { projectId, members }: { projectId: string, members: { userId: string, role: ProjectRole }[]}, context: GraphQLContext) => {
        const currentUserId = context.user?.id;
        if (!currentUserId) throw new ForbiddenError("Authentication required.");
    ...
    updateProjectMemberRole: async (_: any, { memberId, role }: { memberId: string, role: ProjectRole }, context: GraphQLContext) => {
        const userId = context.user?.id;
        if (!userId) throw new ForbiddenError("Authentication required.");

        const memberToUpdate = await prisma.projectMember.findUnique({ where: { id: memberId }});
        if (!memberToUpdate) throw new UserInputError("Project member not found.");

        await checkProjectAdmin(memberToUpdate.projectId, userId, context.prisma);

        if (memberToUpdate.role === 'OWNER') throw new ForbiddenError("Cannot change the role of the project owner.");
    ...
    removeProjectMembers: async (_: any, { memberIds }: { memberIds: [string] }, context: GraphQLContext) => {
        const userId = context.user?.id;
        if (!userId) throw new ForbiddenError("Authentication required.");
