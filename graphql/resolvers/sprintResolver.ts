...
import { SprintStatus , SprintUi } from "@/types/sprint";
...
export const sprintResolvers = {
  Mutation: {
    createSprint: async (
      _parent: any,
      { input }: { input: { projectId: string; name: string; description?: string; startDate: string; endDate: string; status?: SprintStatus } },
      ...
      const project = await prisma.project.findUnique({ // Use imported prisma
        where: { id: projectId },
      });
      ...
      const newSprint = await prisma.sprint.create({ // Use imported prisma
        data: {
          projectId,
          name,
          description,
      ...
      const sprintDetails = await prisma.sprint.findUnique({ // Use imported prisma
      ...
      return {
        ...
        status: sprintDetails.status, // Directly use Prisma's SprintStatus
        tasks: mappedTasks,
      };
    },
    ...
    updateSprint: async (
      _parent: any,
      { input }: { input: { id: string; name?: string; description?: string; startDate?: string; endDate?: string; isCompleted?: boolean; status?: SprintStatus } },
      // context: GraphQLContext // Remove context argument
    ): Promise<SprintUi> => {
      ...
      const existingSprint = await prisma.sprint.findUnique({ // Use imported prisma
        where: { id },
      });
      ...
      const updatedSprint = await prisma.sprint.update({ // Use imported prisma
        where: { id },
        data: updateData,
        include: {
          tasks: {
    ...
    deleteSprint: async (
      ...
      const deletedSprint = await prisma.sprint.delete({ // Use imported prisma
        where: { id },
        include: {
          tasks: { // Include tasks to return the full SprintDetails as per GraphQL schema
            select: {
              id: true,
              title: true,
              description: true,
