...
//               data: { type: 'COMMENT_ADDED', data: { content: content.substring(0, 50) + '...' }, userId: authorId, taskId }
//           })
//       ]);
...
updateGanttTask: async (_parent: unknown, { input }: { input: UpdateGanttTaskInput }, context: GraphQLContext) => { 
  ...
  // This logic currently only handles the 'Task' model. 
  // You would need to add logic for 'Milestone' if 'type' can be 'MILESTONE'.
  const updatedGanttTask = await prisma.task.update({
      where: { id },
      data: {
          title: name,
  ...
  return {
      ...
      assignee: updatedGanttTask.assignee,
      // --- FIX ---
      // Add these two lines to ensure the response includes the fields
      // that the client-side cache depends on.
