...
export function TaskDetailSheet({
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
        ...
        if (editingTaskLocal.assignee?.id !== taskDetails.assignee?.id) updates.assignee = editingTaskLocal.assignee;
  ...
  return (
    <Sheet open={!!sheetTask} onOpenChange={(open) => (!open ? onClose() : null)}>
      <SheetContent side="right" className="w-full sm:max-w-[1200px] bg-gray-100 border-l p-0 flex flex-col h-full max-h-screen">
        {taskDetailsLoading ? (
