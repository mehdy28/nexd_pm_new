...
interface NewTicketFormProps {
  onBack: () => void;
  onSubmit: (data: { subject: string; priority: 'LOW' | 'MEDIUM' | 'HIGH'; message: string }) => Promise<void>;
}
...
export function NewTicketForm({ onBack, onSubmit }: NewTicketFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (subject.trim() && message.trim()) {
      await onSubmit({ subject, priority, message });
    }
  };
  
  const priorityColors: Record<string, string> = { LOW: 'bg-green-500', MEDIUM: 'bg-yellow-500', HIGH: 'bg-red-500' };
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2 border rounded-md p-2 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring has-[:focus-visible]:ring-offset-2">
              <Input placeholder="e.g., Unable to access project dashboard" value={subject} onChange={(e) => setSubject(e.target.value)} required className="flex-1 bg-transparent border-0 resize-none focus-visible:ring-0 focus-visible:ring-offset-0 p-0" />
            </div>
          </div>
          <div className="space-y-2">
