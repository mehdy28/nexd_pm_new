...
export function KanbanCard({
  ...
  if (card.editing) {
    return (
      <div className="self-start bg-card rounded-xl border shadow-medium p-4 transition-all duration-200">
        <Input
          ref={titleRef}
          value={title}
  ...
  return (
    <div
      className="w-full self-start bg-card text-left rounded-xl border shadow-soft hover:shadow-medium hover:scale-[1.02] transition-all duration-200 p-4 group"
      onClick={onOpen}
    ...
    >
      ...
      <div className="text-sm font-semibold text-foreground leading-tight mb-3 group-hover:text-primary transition-colors break-words">
        {card.title}
      </div>
