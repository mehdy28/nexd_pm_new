...
export function KanbanBoard({
  ...
  useEffect(() => {
    console.log("[KanbanBoard] useEffect: initialColumns changed. Setting columns.", initialColumns);
  ...
  const handleUpdateTask = useCallback(async (sectionId: string, taskId: string, updates: Partial<TaskUI>) => {
    await updateCard(taskId, updates, currentSprintId || null);
  }, [updateCard, currentSprintId]);

  const handleDeleteRequest = useCallback((sectionId: string, task: TaskUI) => {
      deleteCard(task.id, currentSprintId || null);
      closeDrawer();
  }, [deleteCard, closeDrawer, currentSprintId]);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    console.log(`[KanbanBoard] handleDragEnd started. activeId: ${activeId}, overId: ${overId}, type: ${activeType}`);

    setOverlay(null);
    if (!overId || activeId === overId) {
      console.log("[KanbanBoard] handleDragEnd aborted: no-op (no overId or activeId === overId).");
      return;
    }
    ...
    if (activeType === "column") {
      setColumns((currentColumns) => {
        ...
        updateColumn(activeId, undefined, toIndex, currentSprintId || null);
    ...
    if (activeType === "card") {
      ...
      if (!fromColumnId) {
        console.error(`[KanbanBoard] handleDragEnd error: Could not find fromColumnId for cardId: ${activeId}`);
        return;
      }
      ...
      if (!toColumnId) {
        console.error(`[KanbanBoard] handleDragEnd error: Could not find toColumnId for overId: ${overId}`);
        return;
      }
      ...
      console.log(`[KanbanBoard] Card move details: fromColumnId=${fromColumnId}, toColumnId=${toColumnId}, fromIndex=${fromIndex}, toIndex=${toIndex}`);


      if (fromColumnId === toColumnId && fromIndex === toIndex) {
        console.log("[KanbanBoard] handleDragEnd aborted: card did not change position.");
        return;
      }
      
      setColumns((prev) => {
        console.log("[KanbanBoard] setColumns (optimistic update) started. Previous state:", JSON.parse(JSON.stringify(prev)));
        const next = prev.map((c) => ({ ...c, cards: [...c.cards] }));
        const fromCol = next.find((c) => c.id === fromColumnId)!;
        const toCol = next.find((c) => c.id === toColumnId)!;

        if (fromColumnId === toColumnId) {
            fromCol.cards = arrayMove(fromCol.cards, fromIndex, toIndex);
        } else {
            const [movedCard] = fromCol.cards.splice(fromIndex, 1);
            toCol.cards.splice(toIndex, 0, movedCard);
        }
        console.log("[KanbanBoard] setColumns (optimistic update) finished. Next state:", JSON.parse(JSON.stringify(next)));
        return next;
      });
      
      const mutationPayload = { sectionId: toColumnId, sprintId: currentSprintId };
      console.log(`[KanbanBoard] Calling updateCard with: cardId=${activeId}, payload=`, mutationPayload, `currentSprintId=${currentSprintId}`);
      updateCard(activeId, mutationPayload, currentSprintId || null);
    }
  }
  ...
  const handleAddCard = useCallback(async (columnId: string, title: string) => await createCard(columnId, title, currentSprintId || null), [createCard, currentSprintId]);
  const handleUpdateColumnTitle = useCallback(async (columnId: string, title: string) => {
    const currentColumn = columns.find(c => c.id === columnId);
    await updateColumn(columnId, title, currentColumn?.order, currentSprintId || null);
  }, [updateColumn, columns, currentSprintId]);
  const handleDeleteColumn = useCallback(async (columnId: string) => await deleteColumn(columnId, currentSprintId || null), [deleteColumn, currentSprintId]);
  const handleDeleteCard = useCallback(async (cardId: string) => {
    await deleteCard(cardId, currentSprintId || null);
    setDrawerOpen(false);
  }, [deleteCard, currentSprintId]);
  ...
  return (
    <div className="page-scroller">
      ...
      <div className="flex items-center  pr-6 pl-6 pt-3 gap-3">
        <Button onClick={() => createColumn("New Column", undefined, currentSprintId || null)} className="bg-[#4ab5ae] text-white h-9 rounded-md" disabled={isMutating}>
                  ...
                  {column.cards.map((card) => (
                    <KanbanSortableCard
                      ...
                      onFinishInline={(patch) => {
                        updateCard(card.id, patch, currentSprintId || null); 
