...
type VariableBlock = { type: 'variable'; id: string; varId?: string; placeholder: string; name?: string };
type Block = TextBlock | VariableBlock;

// Utility for deep comparison of arrays of objects - CRITICAL FIX HERE
function deepCompareBlocks(arr1: Block[], arr2: Block[]): boolean {
  if (arr1.length !== arr2.length) {
    // console.log("deepCompareBlocks: Lengths differ", arr1.length, arr2.length);
    return false;
  }
  for (let i = 0; i < arr1.length; i++) {
    ...
    // Compare type first
    if (b1.type !== b2.type) {
      // console.log(`deepCompareBlocks: Type differs at index ${i}. b1.type=${b1.type}, b2.type=${b2.type}`);
      return false;
    }

    // Compare content based on type
    if (b1.type === 'text') {
      const textBlock1 = b1 as TextBlock;
      const textBlock2 = b2 as TextBlock;
      if (textBlock1.value !== textBlock2.value) {
        // console.log(`deepCompareBlocks: Text value differs at index ${i}. b1.value="${textBlock1.value}", b2.value="${textBlock2.value}"`);
        return false;
      }
    } else { // type === 'variable'
      const varBlock1 = b1 as VariableBlock;
      const varBlock2 = b2 as VariableBlock;
      if (varBlock1.placeholder !== varBlock2.placeholder || varBlock1.name !== varBlock2.name) {
        // console.log(`deepCompareBlocks: Variable content differs at index ${i}. b1.placeholder=${varBlock1.placeholder}, b2.placeholder=${varBlock2.placeholder}`);
        return false;
      }
      // Note: varId is an internal ID from `prompt.variables` and might be useful to compare
      // if you rely on specific variable instances. For simple content comparison, placeholder/name are usually enough.
      // If `varId` is stable across re-parses for the *same conceptual variable*, you could include it:
      // if (varBlock1.varId !== varBlock2.varId) return false;
    }
    // We explicitly IGNORE the 'id' field generated by uid() for this comparison
    // because it will always be different on re-parse and would trigger false negatives.
  }
  // console.log("deepCompareBlocks: Arrays are deeply identical by content.");
  return true;
}
...
// Type definitions moved up for deepCompareBlocks
// type TextBlock = { type: 'text'; id: string; value: string };
// type VariableBlock = { type: 'variable'; id: string; varId?: string; placeholder: string; name?: string };
// type Block = TextBlock | VariableBlock;
...
function parseContentToBlocks(content: string, variables: PromptVariable[]): Block[] {
  ...
  // Ensure there's always at least one text block, even if empty,
  // to make the UI editable when content is entirely removed.
  if (finalBlocks.length === 0) {
      return [{ type: 'text', id: uid('t-initial-empty-fallback'), value: '' }];
  }
  // Trim leading/trailing empty text blocks only if there's more than one block.
  // This prevents an empty editor from being completely blank.
...
function EditorPanel({
  ...
  }, [prompt.content, prompt.variables]); // REMOVED `blocks` from dependencies to break the potential loop
  ...
  }, [logBlocks]); // Removed `blocks` from dependencies
      ...
      >
        ...
        >
            ...
            <div
                ...
                // MODIFIED: Added `min-w-0` and `overflow-wrap` for proper text wrapping in flex container
                className="flex-1 min-h-[40px] text-sm outline-none w-full whitespace-pre-wrap py-2 min-w-0 overflow-wrap-anywhere" 
