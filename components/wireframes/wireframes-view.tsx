...
export function WireframesView({ projectId }: { projectId?: string }) {
  ...
  const handleRenameInputKeyDown = useCallback((e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      handleConfirmRename();
  ...
  const deleteModalDescription = deleteTarget && Array.isArray(deleteTarget) ? `You are about to permanently delete ${deleteTarget.length} wireframes. This action cannot be undone.` : "You are about to permanently delete this wireframe. This action cannot be undone.";
  ...
  return (
    <>
        ...
        <div className="saas-card flex flex-1 flex-col overflow-hidden p-4">
          <div className="mb-4 shrink-0 border-b pb-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto] md:items-center">
              <Input value={search} onChange={(e) => setSearch(e.target.value)} className="h-9 sm:w-[280px]" placeholder="Search wireframes..." />
              <div className="flex items-center justify-start gap-2 md:justify-end">
                {selectedCount > 0 && (
                  <>
                    <Button variant="outline" className="h-9 bg-transparent" onClick={handleExportSelected}>Export ({selectedCount})</Button>
                    <Button variant="destructive" className="h-9" onClick={handleBulkDelete}><Trash2 className="mr-1 h-4 w-4" />Delete selected</Button>
                  </>
                )}
                <Button className="h-9 btn-primary" onClick={handleCreateNew}><Plus className="mr-1 h-4 w-4" />New wireframe</Button>
              </div>
            </div>
          </div>
          ...
          <div className="relative flex-1 overflow-y-auto">
            ...
            {pageItems.length > 0 ? (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {pageItems.map((w) => (
                  <article key={w.id} className="group relative overflow-hidden rounded-lg border">
                    <div className="absolute left-2 top-2 z-10"><Checkbox checked={!!selected[w.id]} onCheckedChange={(v) => toggleSelect(w.id, !!v)} aria-label="Select wireframe" /></div>
                    <div onClick={() => setEditingWireframeId(w.id)} title="Open wireframe" className="block cursor-pointer">
                      <div className="aspect-[16/10] w-full bg-slate-50"><img src={w.thumbnail || "/placeholder.svg"} alt={`${w.title} preview`} className="h-full w-full object-cover" /></div>
                    </div>
                    <div className="flex items-center justify-between px-3 py-2">
                      ...
                      <div className="flex items-center gap-1">
                        <Button size="sm" variant="ghost" className="h-8 px-2" onClick={() => handleStartRename(w.id, w.title)} title="Rename"><PencilLine className="h-4 w-4" /></Button>
                        <Button size="sm" variant="ghost" className="h-8 px-2 text-red-600 hover:bg-red-50 hover:text-red-700" onClick={() => setDeleteTarget(w.id)} title="Delete"><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </div>
                  </article>
                ))}
              ...
              <div className="flex h-full items-center justify-center rounded-md border border-dashed text-center text-sm text-slate-500">
                <div><p>No wireframes found.</p><p className="mt-1">Click “New wireframe” to get started.</p></div>
              </div>
            ) : null}
          </div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>{deleteModalDescription}</AlertDialogDescription></AlertDialogHeader>
